...
from app_fastapi.prompts.extract_keyword import (
    ...
    ko_en_prompt as extract_ko_en_prompt,
)
import os, json
import pandas as pd
...
async def http_post_ko_en():
    ...
    for each in chunk_files:
        ...
        if each.endswith(".txt"):
            ...
            client = Clients.client_openai()
            chain = extract_ko_en_prompt | client
            response = chain.invoke({"long_text": full_chunk})
            try:
                res_dict = json.loads(response.content)
            except:
    ...
    if all_dict:
        ...
        df.to_excel("storage/chunks/keyword_ko_en.xlsx", index=False)
